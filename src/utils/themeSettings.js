...
export const EXTERNAL_THEMES_KEY = "todo-app.external-themes";
...
export const CSS_VAR_GROUPS = [
  {
    label: "Backgrounds",
    vars: ["--bg", "--surface", "--surface-strong", "--surface-muted"],
  },
  ...
  {
    label: "Inputs & Modal",
    vars: ["--input-bg", "--input-border", "--modal-bg"],
  },
...
export function loadExternalThemes() {
  try {
    const stored = localStorage.getItem(EXTERNAL_THEMES_KEY);
...
export function saveExternalThemes(externalThemes) {
  localStorage.setItem(EXTERNAL_THEMES_KEY, JSON.stringify(externalThemes));
}

export function getAllThemesWithExternal() {
  return [...themes, ...loadCustomThemes(), ...loadExternalThemes()];
}
